...
- [x] The JWT authentication filter (`JwtAuthenticationFilter`, a Gateway `GlobalFilter`) never populates the `ReactiveSecurityContextHolder`, while `SecurityConfig` requires `.anyExchange().authenticated()` with `httpBasic()` as the only configured mechanism (no `UserDetailsService`). The two authentication layers appear uncoordinated — verify end-to-end whether JWT-authenticated requests actually pass the Spring Security chain, then reconcile the two (either wire the JWT filter into the reactive security context, or remove the unused `httpBasic()` fallback). Fixed: confirmed via `SecurityChainIntegrationTest` that `httpBasic()` rejected all traffic before the JWT filter ran (Spring Security's `WebFilter` executes before Gateway's routing/`GlobalFilter`s). `SecurityConfig` now only disables CSRF and permits all exchanges; `JwtAuthenticationFilter` is the sole authentication gate for gateway-routed traffic.
- [x] ~~Configured route URIs (`http://products-service`, `http://orders-service`, `http://secure-service`) are placeholder hostnames with no service discovery or `lb://` load-balancer scheme — the gateway is not wired to any real backend today.~~ Not a bug: on Kubernetes/OpenShift, a `Service` named e.g. `products-service` gets a stable in-cluster DNS name and ClusterIP, and kube-proxy already load-balances across pod replicas at L4 — plain `http://products-service` URIs are the correct, idiomatic way to reach backends there. The `lb://` scheme is only needed for Spring Cloud LoadBalancer against a client-side registry (Eureka/Consul), which is an alternative to k8s-native discovery, not a requirement on top of it. The actual gap is that no Kubernetes/OpenShift manifests exist yet to define those Services — tracked under "Deployment / Ops" below.
...
- [ ] No Dockerfile, docker-compose file, Kubernetes/OpenShift manifests, or CI/CD pipeline exist in the repository, despite the project being described (in `pom.xml` and the README) as intended for OpenShift deployment. This is the largest gap between stated purpose and current state. This also includes defining the `Service`/`Deployment` manifests for `products-service`, `orders-service`, and `secure-service` (or renaming the gateway's route URIs to match whatever the real Service names turn out to be) so the plain-hostname routes configured in `application.yml` actually resolve via in-cluster DNS.
...
- [x] Confirm, with real requests against a running instance, whether the Spring Security chain (`.anyExchange().authenticated()` + `httpBasic()`) actually blocks requests that carry a valid JWT but no Basic Auth header. Confirmed and resolved — see Critical section above.
...
- [x] Verify whether the `/actuator/gateway` endpoint (exposed via `management.endpoints.web.exposure.include`) is reachable end-to-end given the current (possibly misconfigured) security chain. Resolved — the Security chain now permits all exchanges (see Critical section above), so `/actuator/gateway` is reachable; it is not gated by `JwtAuthenticationFilter` either since actuator endpoints aren't Gateway routes.
